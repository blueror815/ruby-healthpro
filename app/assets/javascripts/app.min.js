(function($) {
    ...
    p.minBreakpoint = function(alias) {
        ...
        var breakpoint = $('#device-breakpoints div:visible').data(
            'breakpoint');
        return $.inArray(alias, breakpoints) < $.inArray(breakpoint,
            breakpoints);
    };
...
(function(namespace, $) {
    ...
    p._enableEvents = function() {
        ...
        // $('body.menubar-hoverable').on('mouseenter', '#menubar',
        //     function(e) {
        ...
        if ((menuItemClicked === true || offcanvasVisible === false) &&
            ...
            if (menuItemClicked === false) {
                if (this._lastOpened) {
                    ...
                    this._lastOpened.parents('.gui-folder').each(
                        function() {
    ...
    p._handleMenuLinkClick = function(e) {
        var pinned = ($('body').hasClass('menubar-pin') &&
            materialadmin.App.minBreakpoint('md'));
    ...
    p._evalMenuScrollbar = function() {
        ...
        if (parent.hasClass('nano-content') === false) {
            menuScroller.wrap(
                '<div class="nano"><div class="nano-content"></div></div>'
            );
        }
        var height = $(window).height() - menu.position().top - menu.find(
            '.nano').position().top;
...
(function(namespace, $) {
    ...
    p._handleOffcanvasOpen = function(btn) {
        ...
        this._useBackdrop = (btn.data('backdrop') === undefined) ? true :
            btn.data('backdrop');
    ...
    p.openOffcanvas = function(id) {
        this.closeOffcanvas();
        $(id).addClass('active');
        var leftOffcanvas = ($(id).closest('.offcanvas:first').length >
            0);
    ...
    p._addBackdrop = function() {
        if ($('#base > .backdrop').length === 0 && $('#base').data(
            'backdrop') !== 'hidden') {
    ...
    p.toggleBodyScrolling = function() {
        ...
        if ($('.offcanvas-pane.active').length > 0 && this._useBackdrop) {
            ...
            var bodyPad = parseInt(($('body').css('padding-right') || 0),
                10);
            if (scrollbarWidth !== bodyPad) {
                $('body').css('padding-right', bodyPad + scrollbarWidth);
                $('.headerbar').css('padding-right', bodyPad +
                    scrollbarWidth);
            }
        } else {
    ...
    p.evalScrollbar = function() {
        ...
        var height = $(window).height() - menu.find('.nano').position()
            .top;
        var scroller = menuScroller.closest('.nano');
        scroller.css({
            height: height
...
(function(namespace, $) {
    ...
    p.addCardLoader = function(card) {
        var container = $('<div class="card-loader"></div>').appendTo(
            card);
    ...
    p.toggleCardCollapse = function(card, duration) {
        ...
        card.find('.card-body').slideToggle(duration, function() {
            if (dispatched === false) {
                $('#COLLAPSER').triggerHandler(
                    'card.bb.collapse', [!$(this).is(
                        ":visible")]);
                dispatched = true;
            }
        });
...
(function(namespace, $) {
    ...
    p._initFloatingLabels = function() {
        var o = this;
        $('.floating-label .form-control').on('keyup change', function(
            e) {
    ...
    p._initScroller = function() {
        ...
        materialadmin.App.callOnResize(function() {
            $.each($('.scroll-xs'), function(e) {
                ...
                if (materialadmin.App.minBreakpoint(
                    'xs')) {
                    materialadmin.AppVendor.removeScroller(
                        holder);
                } else {
                    materialadmin.AppVendor.addScroller(
                        holder);
                }
            });
